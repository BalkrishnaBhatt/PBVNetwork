...
export const getClientDimensionList = navigation => {
  let url = 'pbv/v1/client_dimension';
  const config = {
    headers: {
  ...
  return async dispatch => {
    axiosInstance
      ...
      .then(function (response) {
        console_log(
          'getClientDimensionList response: ',
...
export const get_lawyer_role = list => {
  return {
    type: actionTypes.GET_CLIENT_DIMENSION_LIST,
    list,
  };
};
